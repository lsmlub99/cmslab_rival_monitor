...
위 경쟁 동향 전체를 종합해 시장을 읽고, 우리(셀퓨전씨)에게 맞는 실행 조언을 주세요.
출력 규칙: 마크다운 볼드(**)·번호목록 쓰지 말 것. 각 항목은 "- "로 시작하는 짧은 문장. 머리말은 `### `.

### 시장 흐름
지금 경쟁 시장을 관통하는 핵심 흐름 2~3개를 "- "로. 각 줄은 [무엇이 몰리는가] + [근거 브랜드·채널·시장]. 추상어 금지.

### 핵심 위협·기회
막연히 "경쟁 심화" 같은 말 금지. **날카롭게** 구체적으로:
- 위협은 "누가(경쟁사) + 무엇을(구체 행동) + 그게 왜 셀퓨전씨에 직접 타격인지(우리 주력=선케어/더마, 베트남·중국·일본과 겹치는 지점) + 시급도"까지 한 줄에.
- 기회도 "어떤 빈틈/트렌드를 + 우리의 어떤 강점(임상·더마 선케어)으로 + 어디서 선점"까지 한 줄에.
- 각 2~3개, 우리와 무관한 일반 동향은 빼고 겹치는 것만.

### 셀퓨전씨 액션
위 위협·기회에 1:1로 대응하는 실행 액션 2~3개를 "- "로. 각 액션 = [구체 행동] + [대상 시장/채널] + [노리는 효과]. "강화한다/주력한다" 같은 뭉툭한 동사 말고 실제 실행 단위로."""
